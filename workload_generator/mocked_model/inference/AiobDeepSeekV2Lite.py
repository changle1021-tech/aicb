...
def _bench_fp8_gemm(m, k, n):
    try:
        ...
        from utils.deepgemm_utils import construct
    except ModuleNotFoundError as exc:
        raise RuntimeError(
            "FP8 AIOB profiling requires DeepGEMM. Either install deep_gemm "
            "or set aiob_dtype to bfloat16 in the model config."
        ) from exc
    ...
    return bench_kineto(gemm, "fp8_gemm", suppress_kineto_output=True)
...
class DeepSeekV2LiteMLP(torch.nn.Module):
...
class DeepSeekV2LiteMOE(torch.nn.Module):
    def __init__(self, args):
        super().__init__()
        self.batch_size = args.micro_batch
        self.hidden_size = args.hidden_size
    ...
    def _up_gate(self, m):
        ...
        try:
            from utils.deepgemm_utils import bench_contiguous, bench_masked
        except ModuleNotFoundError as exc:
            raise RuntimeError(
                "FP8 MoE profiling requires DeepGEMM. Set aiob_dtype to "
                "bfloat16 to use the dependency-free BF16 path."
            ) from exc
        phase = getattr(self.args, "phase", InferencePhase.DECODE.value)
        bench = (
            bench_contiguous
